...
def construct_calibration_parser(parser):
    """
    Construct the parser for calibration
    """
    ...
    calib_parser_list = [de_parser, ga_parser,
                         nsga2_parser, nsde_parser, gde3_parser]
    for calibration_parser in calib_parser_list:
    ...
    calib_parser_list = [de_parser, ga_parser,
                         nsga2_parser, nsde_parser, gde3_parser, direct_parser]
    for calibration_parser in calib_parser_list:
...
def construct_sa_parser(parser):
    """
    Construct the parser for sensitivity analysis
    """
    subparsers = parser.add_subparsers(
    ...
    for sa_analsis_parser in [fast_parser, sobol_parser]:
        sa_analsis_parser.add_argument(
            "--num-samples",
            type=int,
            default=80,
...
def _get_parser(args_to_parse):
    ...
    if _check_for_args(args_to_parse, args_to_check):
        construct_calibration_parser(parser)
...
def _kwargs_prompt():
    ...
    for key, (prompt, default) in keys.items():
        ...
        elif key in ["intersection_points_xy", "intersection_points_lonlat"]:
            if resp.strip() == "":
                resp = default
